...
\input{tables/novelty_comparison_table}
...
\input{tables/experiment_config_table}
\input{tables/revision_experiment_protocol_table}
...
\input{tables/baseline_metric_matrix}
...
\input{tables/e1_flen_screening_table}
...
\subsection{Measured MultiEURLEX multilingual screening}

We additionally ran a controlled MultiEURLEX screening with English--German, English--French, English--Spanish, and English--Polish language clients. For each target language pair, the remaining three language clients trained a Qwen2.5-0.5B-Instruct LoRA adapter for three federated rounds using 500 training examples per client. The evaluation uses 500 validation and 500 test examples per language and matched seeds 41--43. A single decision threshold is selected by micro-$F_1$ on source-language validation sets only; the held-out language does not participate in threshold selection. All 36 runs completed on GPU and passed checks for finite metrics, interior calibration thresholds, prediction reconstruction, identifiers, and artifact completeness.

\input{tables/multieurlex_results_table}

Table~\ref{tab:multieurlex-results} shows no FedProx advantage over FedAvg. Averaging the four held-out-language scores within each seed, FedProx changes micro-$F_1$ by $-0.0015$ (95\% Student-$t$ interval $[-0.0107,0.0077]$) and macro-$F_1$ by $-0.0011$ ($[-0.0156,0.0135]$). The $\lambda=0$ control exactly reproduces FedAvg because equal client sample counts and the absence of calibrated target-specific conflict evidence reduce this control to sample-weighted averaging. This equality is an implementation check, not evidence for the conflict mechanism. Each run communicates 12.81~MiB of logical adapter payload per round, or 38.42~MiB over three rounds, excluding serialization and transport overhead.

These experiments measure held-out-language behavior within a common EU regulatory authority and therefore cannot establish independent national-jurisdiction rule migration. They also do not evaluate FLEN: the runner rejects that method without an expert-adjudicated target-conflict manifest. Moreover, because this screening lacks a local-only or unadapted reference, its non-zero held-out score demonstrates evaluation feasibility but does not by itself identify a positive transfer gain. Five-seed long runs and those references remain necessary before an effectiveness claim.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{multieurlex_heldout_micro_f1.pdf}
    \caption{Held-out-language MultiEURLEX micro-$F_1$ over seeds 41--43; error bars show one standard deviation. The $\lambda=0$ control overlaps FedAvg exactly, while FedProx has no consistent advantage. All language pairs share one EU regulatory authority scope.}
    \label{fig:multieurlex-heldout}
\end{figure}
...
The current evidence establishes that target-conditioned Flower/PEFT aggregation executes reproducibly, that conflict weighting changes client contributions, and that unguarded local residuals are a concrete failure mode mitigated by train-internal selection. The MultiEURLEX screening additionally establishes a reproducible held-out-language evaluation path, but FedProx does not improve its average result and the experiment contains no calibrated FLEN condition. The three-seed studies do not demonstrate statistically reliable aggregate or worst-client gains, and neither heuristic CaseHOLD partitions nor common-authority language partitions validate legal-conflict semantics or independent national-jurisdiction transfer. They also provide no formal privacy or multi-agent evidence, so the corresponding claims remain outside the scope of the measured study.
