...
## 2026-09-15 MultiEURLEX Matched Screening

- Jobs `107186`--`107193` and `107196`--`107199` completed with exit code `0:0`.
  The accepted matrix contains FedAvg, FedProx, and the lambda-zero control over four
  held-out languages and seeds 41--43, for 36 accepted runs and 72,000 predictions.
- Every run used CUDA, completed three rounds, produced 2,000 predictions, selected an
  interior threshold using source-language validation micro-F1, and passed probability,
  identifier, threshold-reconstruction, configuration, and artifact checks.
- Mean held-out micro-F1 across the four language targets is `0.3360` for FedAvg,
  `0.3345` for FedProx, and `0.3360` for lambda-zero. The paired FedProx-minus-FedAvg
  difference is `-0.0015` with a three-seed 95% Student-t interval of
  `[-0.0107, 0.0077]`. Mean macro-F1 changes by `-0.0011`, interval
  `[-0.0156, 0.0135]`.
- Lambda-zero exactly reproduces FedAvg in this equal-size, common-authority setting.
  FedProx's logged proximal penalty is positive in all 12 runs, confirming that its
  implementation is active, but it provides no consistent utility gain.
- These are development results: three seeds, three rounds, and 500 training examples per
  client. They do not test FLEN, because no expert-adjudicated target-conflict manifest is
  available. They support a reproducible multilingual EU-law holdout evaluation only, not
  beneficial transfer or independent national-jurisdiction rule migration.
- Reproducible summaries are generated by `scripts/analyze_multieurlex_comparison.py` under
  `outputs/experiment_analysis/multieurlex_comparison/`. Each run records 12.81 MiB of
  logical adapter payload per round and 38.42 MiB over three rounds.
