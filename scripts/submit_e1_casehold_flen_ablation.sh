...
for METHOD_VALUE in conflict_only personalization_only; do
  for SEED_VALUE in ${SEEDS_VALUE}; do
    if [[ "${METHOD_VALUE}" == "conflict_only" ]]; then
      SHARED_STEPS=10
      LOCAL_STEPS=0
